...
@jit(nopython=True)
def boris_step(dt, iterold, rold, vold, Bfield, Efield, m, q):
    #B is a 3 element array
    #dt is a constant
    #other values are updated outside the loop

    half_electrical_impulse = q * dt * Efield / (2 * m)
    v_minus = vold + half_electrical_impulse

    t_help = q * dt * Bfield / (2 * m)
    v_prime = v_minus + np.cross(v_minus, t_help)

    s_help = 2 * t_help / (1 + np.linalg.norm(t_help) ** 2)
    v_plus = v_minus + np.cross(v_prime, s_help)

    vnew = v_plus + half_electrical_impulse
    rnew = rold + dt*vnew
    #Bnew = B(rnew, self.iter * self.dt)
    ...
    def step(self, B, noOfSteps, E=Fields.nullField):
        ...
        for block in range(nBlocks+1):
            ...
            for ii in range(0, noOfSteps):
                ...
                if self.outOfBounds is True or current_radius >= self.radius_max or z <= self.z_min or z >= self.z_max:
                    # set flag to true
                ...
                rnew, vnew = boris_step(self.dt, self.iter, np.asarray(self.r[ii]), np.asarray(self.v[ii]),
                np.array(B(self.r[ii], self.iter * self.dt)), np.array([0,0,0]), m, q)

                self.v.append(vnew)
                self.r.append(rnew)
                self.Bfield.append(B(rnew, self.iter * self.dt))
